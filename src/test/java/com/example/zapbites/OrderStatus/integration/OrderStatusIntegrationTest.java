...
public class OrderStatusIntegrationTest {
    ...
    void getAll() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/order_status")).andExpect(status().isOk()).andReturn(); //mvcResult is the response body of the get call.
    ...
    void getById() throws Exception {
        var expectedResult = objectMapper.readValue(TestHelper.readJsonFile("/Testfiles/OrderStatusTestFiles/get_by_id_3.json"), OrderStatus.class);

        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/order_status/3")).andExpect(status().isOk()).andReturn();
    ...
    @Test
    @DisplayName("test getting order status with id 99 - does not exist")
    void getByIdNonExistent() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/order_status/99")).andExpect(status().isNotFound());
    }
    ...
    void updateById() throws Exception {
        var requestBody = TestHelper.readJsonFile("/Testfiles/OrderStatusTestFiles/update_OrderStatus_with_id_1_request.json");
        var responseBody = TestHelper.readJsonFile("/Testfiles/OrderStatusTestFiles/update_OrderStatus_with_id_1_response.json");

        var expectedResult = objectMapper.readValue(responseBody, OrderStatus.class);

        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.put("/order_status/1").content(requestBody).contentType(MediaType.APPLICATION_JSON_VALUE)).andExpect(status().isOk()).andReturn();
    ...
    void createOrderStatus() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.post("/order_status").content(requestBody).contentType(MediaType.APPLICATION_JSON_VALUE)).andExpect(status().isCreated()).andReturn();
    ...
    void deleteAnExistingOrderStatus() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.delete("/order_status/3")).andExpect(status().isNoContent());
    }
}